...
def get_most_common_seqs(path, n_views):
    data = dataset.DataSet(path)
    reconstructions = data.load_reconstruction()
    seq_dict = {}
    seq_dict_most_common = [[] for i in range(n_views)]
    ...
    for idx, key in enumerate(seq_dict_count[:n_views]):
...
def get_all_images(path, n_views):
    data = dataset.DataSet(path)
    reconstructions = data.load_reconstruction()
    ...
    seqs = [[shot.id for shot in shots] for i in range(n_views)]
    return seqs
