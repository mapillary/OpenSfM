...
class CadView():
    def __init__(
        ...
        server_thread = Thread(target=self.run_server, args=(path_cad_model, q, pipe_write))
        server_thread.start()
    ...
    def run_server(self, path_cad_model, q, pipe_write):
        ...
        @cad_app.route('/')
        def hello_world():
            return 'Hello, World! This would be the '

        @cad_app.route('/post', methods=['POST'])
        def post_example():
    ...
    def process_message_from_cad_view(self, data):
        ...
        if command == "add_or_update_point_observation":
            self.add_remove_update_point_observation(data["xyz"])
    ...
    def add_remove_update_point_observation(self, point_coordinates=None):
        ...
        if point_coordinates:
            ...
            print(f"Added {point_coordinates} observation for {active_gcp} in {self.path_cad_model}")
