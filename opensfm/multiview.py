...
EARTH_RADIUS = 6371000  # This is just an approximation. The earth is not a sphere.

def geocentricFromLatLon(lat, lon, alt):
    '''Geocentric XYZ from latitude, longitude and altitude.

    This assumes a simple spherical model of the Earth.
    The alt is added to the Earth radius.

    >>> np.allclose(geocentricFromLatLon(0, 0, 0), [EARTH_RADIUS, 0, 0])
    True
    >>> np.allclose(geocentricFromLatLon(0, 90, 0), [0, EARTH_RADIUS, 0])
    True
    >>> np.allclose(geocentricFromLatLon(90, 0, 0), [0, 0, EARTH_RADIUS])
    True
    '''
    r = EARTH_RADIUS + alt;
...
def latLonFromGeocentric(x, y, z):
    '''Latitud, longitud and altitud from geocentric XYZ.

    >>> x, y, z = EARTH_RADIUS, EARTH_RADIUS, EARTH_RADIUS
    >>> lat, lon, alt = latLonFromGeocentric(x, y, z)
    >>> np.allclose(geocentricFromLatLon(lat, lon, alt), [x, y, z])
    True
    '''
...
def geocentricFromTopocentricTransform(reference_lat, reference_lon, reference_height):
    trans = tf.translation_matrix([EARTH_RADIUS + reference_height, 0, 0])
    Rlat = tf.rotation_matrix(math.radians(-reference_lat), [0,1,0])
    Rlon = tf.rotation_matrix(math.radians(reference_lon), [0,0,1])
    return Rlon.dot(Rlat.dot(trans))

def geocentricFromTopocentric(x, y, z, reference_lat, reference_lon, reference_height):
    '''
    >>> geo = geocentricFromTopocentric(0, 0, 0, 0, 0, 0)
    >>> np.allclose(geo, [EARTH_RADIUS,0,0])
    True
    >>> geo = geocentricFromTopocentric(0, 0, 0, 0, 90, 0)
    >>> np.allclose(geo, [0,EARTH_RADIUS,0])
    True
    >>> geo = geocentricFromTopocentric(0, 0, 0, 90, 0, 0)
    >>> np.allclose(geo, [0,0,EARTH_RADIUS])
    True
    '''
    T = geocentricFromTopocentricTransform(reference_lat, reference_lon, reference_height)
    geo = T.dot([x,y,z,1])
    return geo[:3]

def topocentricFromGeocentric(x, y, z, reference_lat, reference_lon, reference_height):
    '''
    >>> lat, lon, h = 20, 30, 1234
    >>> x, y, z = geocentricFromLatLon(lat, lon, h)
    >>> topo = topocentricFromGeocentric(x, y, z, lat, lon, h)
    >>> np.allclose(topo, [0,0,0])
    True
    '''
    T = geocentricFromTopocentricTransform(reference_lat, reference_lon, reference_height)
    topo = np.linalg.inv(T).dot([x,y,z,1])
    return topo[:3]
