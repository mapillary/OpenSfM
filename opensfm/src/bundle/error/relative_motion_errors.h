...
struct RelativeMotionError {
  ...
  Eigen::Matrix<T, 6, 1> Error(const T* const shot_i, const T* const scale,
    ...
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > Ri(shot_i +
                                                 Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > Rj(shot_j +
                                                 Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > ti(shot_i +
                                                 Pose::Parameter::BA_SHOT_TX);
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > tj(shot_j +
                                                 Pose::Parameter::BA_SHOT_TX);
    Eigen::Matrix<T, 6, 1> residual;

    // Compute rotation residual: log( Rij Ri Rj^t )  ->  log( Rij Ri^t Rj)
    const Eigen::Matrix<T, 3, 1> Rij =
        Rtij_.segment<3>(Pose::Parameter::BA_SHOT_RX).cast<T>();
    residual.segment(0, 3) = MultRotations(Rij, (-Ri).eval(), Rj.eval());

    // Compute translation residual: tij - scale * ( tj - Rj Ri^t ti )  ->  tij
    // - scale * Rj^t * (ti - tj)
    const auto tij = Rtij_.segment<3>(Pose::Parameter::BA_SHOT_TX).cast<T>();
...
struct CommonPositionError {
  ...
  bool operator()(const T* const shot_1, const T* const shot_2, T* r) const {
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > R1(shot_1 +
                                                 Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > t1(shot_1 +
                                                 Pose::Parameter::BA_SHOT_TX);
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > R2(shot_2 +
                                                 Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Eigen::Matrix<T, 3, 1> > t2(shot_2 +
                                                 Pose::Parameter::BA_SHOT_TX);
    Eigen::Map<Eigen::Matrix<T, 3, 1> > residual(r);
