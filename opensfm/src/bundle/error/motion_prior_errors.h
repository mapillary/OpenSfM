...
struct LinearMotionError {
  ...
  bool operator()(const T* const shot0, const T* const shot1,
    ...
    Eigen::Map<const Vec3<T> > R0(shot0 + Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Vec3<T> > t0(shot0 + Pose::Parameter::BA_SHOT_TX);
    Eigen::Map<const Vec3<T> > R1(shot1 + Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Vec3<T> > t1(shot1 + Pose::Parameter::BA_SHOT_TX);
    Eigen::Map<const Vec3<T> > R2(shot2 + Pose::Parameter::BA_SHOT_RX);
    Eigen::Map<const Vec3<T> > t2(shot2 + Pose::Parameter::BA_SHOT_TX);
