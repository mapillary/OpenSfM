...
struct PositionPriorError {
  ...
  template <typename T>
  bool operator()(const T* const shot, const T* const bias, T* residuals) const {
    ...
    Eigen::Map<Vec3d> prior_with_bias(position_prior_);
    Eigen::Map<Vec3<T> > res(residuals);
    Eigen::Map<const Vec3<T> > optical_center(shot + Pose::Parameter::TX);

    res = optical_center - ApplySimilarity(scale[0], R.eval(), t.eval(), prior_with_bias.cast<T>().eval());
