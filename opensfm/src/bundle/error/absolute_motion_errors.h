...
struct TiltAngleError {
  ...
  template <typename T>
  bool operator()(const T* const shot, T* residuals) const {
    const T* const R = shot + Pose::Parameter::BA_SHOT_RX;
...
struct RotationPriorError {
  ...
  bool operator()(const T* const shot, T* residuals) const {
    ...
    const T R[3] = {-shot[Pose::Parameter::BA_SHOT_RX],
...
struct TranslationPriorError {
  ...
  template <typename T>
  bool operator()(const T* const shot, T* residuals) const {
    const T mt[3] = {-shot[Pose::Parameter::BA_SHOT_TX],
                     -shot[Pose::Parameter::BA_SHOT_TY],
                     -shot[Pose::Parameter::BA_SHOT_TZ]};
...
struct PositionPriorError {
  ...
  bool operator()(const T* const shot, T* residuals) const {
    residuals[0] =
        T(scale_) * (shot[Pose::Parameter::BA_SHOT_TX] - T(position_prior_[0]));
    residuals[1] =
        T(scale_) * (shot[Pose::Parameter::BA_SHOT_TY] - T(position_prior_[1]));
    residuals[2] =
        T(scale_) * (shot[Pose::Parameter::BA_SHOT_TZ] - T(position_prior_[2]));
    return true;
  }
...
struct HeatmapdCostFunctor {
  ...
  bool operator()(T const* p, T* residuals) const {
    const T x_coor = p[Pose::Parameter::BA_SHOT_TX] - x_offset_;
    const T y_coor = p[Pose::Parameter::BA_SHOT_TY] - y_offset_;
