...
struct BARelativeSimilarityError : public BARelativeMotionError {
  BARelativeSimilarityError(const Eigen::VectorXd& Rtij, double Sij,
                            const Eigen::MatrixXd& scale_matrix)
  ...
  bool operator()(const T* const shot_i, const T* const scale_i,
    ...
    residual.segment(0, 6) =
        BARelativeMotionError::Error(shot_i, scale_j, shot_j);
    if (scale_i[0] == T(0.0)) {
      return false;
    }
...
struct BARelativeRotationError {
  BARelativeRotationError(const Vec3d& Rij, const Eigen::Matrix3d& scale_matrix)
