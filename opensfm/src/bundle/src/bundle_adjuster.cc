...
template <>
struct ErrorTraits<geometry::camera::SphericalCamera> {
...
template <>
struct ErrorTraitsAnalytic<geometry::camera::SphericalCamera, 1> {
...
void BundleAdjuster::Run() {
  ...
  for (auto &i : shots_) {
    ...
    problem.AddParameterBlock(parameter_block,
  ...
  for (auto &i : cameras_) {
    ...
    if (camera.GetProjectionType() == geometry::camera::ProjectionType::DUAL) {
      ...
      if (index >= 0) {
        ceres::CostFunction *transition_barrier =
            new ceres::AutoDiffCostFunction<ParameterBarrier, 1,
                                            geometry::camera::DualCamera::Size>(
  ...
  for (auto &i : rig_instances_) {
    ...
    position_prior->SetConstrainedDataIndexes({Pose::Parameter::BA_SHOT_TX,
                                               Pose::Parameter::BA_SHOT_TY,
                                               Pose::Parameter::BA_SHOT_TZ});
    ceres::CostFunction *cost_function =
  ...
  for (auto &i : cameras_) {
    ...
    geometry::camera::Dispatch<AddCameraPriorlError>(projection_type, i.second,
                                                     &problem);
  }
...
void BundleAdjuster::ComputeReprojectionErrors() {
  ...
  for (auto &observation : point_projection_observations_) {
    const auto projection_type =
        observation.camera->GetValue().GetProjectionType();
    geometry::camera::Dispatch<ComputeResidualError>(
        projection_type, use_analytic_, observation);
  }
  for (auto &observation : point_rig_projection_observations_) {
    const auto projection_type =
        observation.camera->GetValue().GetProjectionType();
    geometry::camera::Dispatch<ComputeRigResidualError>(
        projection_type, use_analytic_, observation);
  }
}
