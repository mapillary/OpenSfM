...
struct RelativeMotion {
  RelativeMotion(const std::string &reconstruction_i, const std::string &shot_i,
    ...
    shot_id_j = shot_j;
    parameters.resize(Pose::Parameter::BA_SHOT_NUM_PARAMS);
    parameters.segment(Pose::Parameter::BA_SHOT_RX, 3) = rotation;
    parameters.segment(Pose::Parameter::BA_SHOT_TX, 3) = translation;
    scale_matrix.resize(Pose::Parameter::BA_SHOT_NUM_PARAMS,
                        Pose::Parameter::BA_SHOT_NUM_PARAMS);
    scale_matrix.setIdentity();
    this->robust_multiplier = robust_multiplier;
  }

  Vec3d GetRotation() const {
    return parameters.segment(Pose::Parameter::BA_SHOT_RX, 3);
  }
  Vec3d GetTranslation() const {
    return parameters.segment(Pose::Parameter::BA_SHOT_TX, 3);
  }
  void SetRotation(const Vec3d &r) {
    parameters.segment(Pose::Parameter::BA_SHOT_RX, 3) = r;
  }
  void SetTranslation(const Vec3d &t) {
    parameters.segment(Pose::Parameter::BA_SHOT_TX, 3) = t;
  }
...
struct RelativeSimilarity : public RelativeMotion {
  RelativeSimilarity(const std::string &reconstruction_i,
      ...
      : RelativeMotion(reconstruction_i, shot_i, reconstruction_j, shot_j,
        ...
        scale(s) {
    scale_matrix.resize(Pose::Parameter::BA_SHOT_NUM_PARAMS + 1,
                        Pose::Parameter::BA_SHOT_NUM_PARAMS + 1);
    scale_matrix.setIdentity();
  }
  double scale;
};
...
struct RelativeSimilarityCovariance {
  static const int Size = Pose::Parameter::BA_SHOT_NUM_PARAMS + 1;
  ...
  void Compute() {
    ...
    for (const auto &p : points) {
      ...
      const auto &y = p[1];
      const auto &z = p[2];
      Eigen::Matrix<double, 3, Pose::Parameter::BA_SHOT_NUM_PARAMS + 1>
          local_jacobian;
      local_jacobian.block(0, Pose::Parameter::BA_SHOT_TX, 3, 3) =
          Eigen::Matrix<double, 3, 3>::Identity();
      local_jacobian.block(0, Pose::Parameter::BA_SHOT_RX, 3, 3) << 0, z, -y,
          -z, 0, x, y, -x, 0;
      local_jacobian.block(0, Pose::Parameter::BA_SHOT_NUM_PARAMS, 3, 1) << x,
          y, z;
