...
PYBIND11_MODULE(pygeometry, m) {
  ...
  py::class_<Camera>(m, "Camera")
      ...
      .def("get_K_in_pixel_coordinates",
           (Eigen::Matrix3d(Camera::*)(int, int)const) &Camera::GetProjectionMatrixScaled
           )
      .def("get_K_in_pixel_coordinates",
           (Eigen::Matrix3d (Camera::*)()const) & Camera::GetProjectionMatrixScaled)
      ...
      .def_property("aspect_ratio", &Camera::GetAspectRatio,
                    &Camera::SetAspectRatio)
      .def_property("distortion", &Camera::GetDistortion,
                    &Camera::SetDistortion)
      .def_property("principal_point", &Camera::GetPrincipalPoint,
                    &Camera::SetPrincipalPoint)
      .def_property("projection_params", &Camera::GetProjectionParams,
                    &Camera::SetProjectionParams)
      .def_property_readonly("projection_type", &Camera::GetProjectionString)
      .def_property_readonly(
          "k1", [](const Camera& c) { return c.GetDistortion()[0]; })
      .def_property_readonly(
          "k2", [](const Camera& c) { return c.GetDistortion()[1]; })
      .def_property_readonly(
          "k3", [](const Camera& c) { return c.GetDistortion()[2]; })
      .def_property_readonly(
          "p1", [](const Camera& c) { return c.GetDistortion()[3]; })
      .def_property_readonly(
          "p2", [](const Camera& c) { return c.GetDistortion()[4]; })
      .def(py::pickle(
          [](const Camera& p) {
            return py::make_tuple(p.GetProjectionParams(), p.GetDistortion(),
                                  p.GetFocal(), p.GetPrincipalPoint(),
                                  p.GetAspectRatio(), p.GetProjectionType(),
      ...
      .def(
          "__copy__", [](const Camera& c, const py::dict& d) { return c; },
