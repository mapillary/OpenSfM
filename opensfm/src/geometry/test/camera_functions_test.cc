...
class FunctionFixture : public ::testing::Test {
 public:
  ...
  struct F1 : public geometry::camera::CameraFunctor<1, 1, 1> {
    template <class T>
  ...
  struct F2 : public geometry::camera::CameraFunctor<1, 2, 1> {
    template <class T>
    static void Apply(const T* in, const T* p, T* out) {
  ...
  struct F3 : public geometry::camera::CameraFunctor<1, 1, 1> {
    template <class T>
    static void Apply(const T* in, const T* p, T* out) {
      out[0] = p[0] * in[0] * in[0];
    }
  ...
  struct F4 : public geometry::camera::CameraFunctor<1, 2, 1> {
    template <class T>
    static void Apply(const T* in, const T* p, T* out) {
      out[0] = p[0] / in[0] - p[1];
    }
...
TEST_F(FunctionFixture, EvaluatesCorrectly) {
  ...
  double evaluated = 0;
  geometry::camera::ComposeFunctions<double, F1, F2, F3, F4>(in, parameters,
                                                             &evaluated);
...
TEST_F(FunctionFixture, EvaluatesDerivativesCorrectly) {
  ...
  double evaluated = 0;
  double jacobian[7];
  geometry::camera::ComposeForwardDerivatives<double, true, F1, F2, F3, F4>(
  ...
  geometry::camera::ComposeFunctions<AScalar, F1, F2, F3, F4>(
...
TEST_F(PoseFixture, EvaluatesCorrectly) {
  ...
  geometry::camera::Pose::Forward(point, rt, transformed);

  /* Use Ceres as groundtruth */
  const double pt[3] = {
      point[0] - rt[geometry::camera::Pose::Tx],
      point[1] - rt[geometry::camera::Pose::Ty],
      point[2] - rt[geometry::camera::Pose::Tz],
  };
  const double Rt[3] = {-rt[geometry::camera::Pose::Rx],
                        -rt[geometry::camera::Pose::Ry],
                        -rt[geometry::camera::Pose::Rz]};
...
TEST_F(PoseFixture, EvaluatesAllDerivativesCorrectly) {
  ...
  geometry::camera::Pose::Forward(point_adiff.data(), rt_adiff.data(),
                                  expected_adiff.data());

  /* Analytic version */
  double transformed[] = {0., 0., 0.};
  double jacobian[size * 3];
  geometry::camera::Pose::ForwardDerivatives<double, true>(
...
TEST_F(PoseFixture, EvaluatesPointDerivativesCorrectly) {
  ...
  double transformed[] = {0., 0., 0.};
  double jacobian[9];
  geometry::camera::Pose::ForwardDerivatives<double, false>(
      &point[0], &rt[0], &transformed[0], &jacobian[0]);
...
class CameraDerivativesFixture : public ::testing::Test {
 public:
  ...
  void RunJacobianEval(const geometry::Camera& camera,
                       geometry::camera::ProjectionType projection,
                       MAT* jacobian) {
    ...
    geometry::camera::Dispatch<geometry::camera::ProjectDerivativesFunction>(
...
TEST_F(CameraDerivativesFixture, ComputePerspectiveAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::PERSPECTIVE,
                  &jacobian);
...
TEST_F(CameraDerivativesFixture, ComputeFisheyeAnalyticalDerivatives) {
  ...
  Eigen::Matrix<double, 2, 6, Eigen::RowMajor> jacobian;
  RunJacobianEval(camera, geometry::camera::ProjectionType::FISHEYE, &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeFisheyeOpencvAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::FISHEYE_OPENCV,
                  &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeFisheye62AnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::FISHEYE62,
                  &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeRadialAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::RADIAL, &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeSimpleRadialAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::SIMPLE_RADIAL,
                  &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeBrownAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::BROWN, &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeSphericalAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::SPHERICAL,
                  &jacobian);
  CheckJacobian(jacobian, size_params);
}
...
TEST_F(CameraDerivativesFixture, ComputeDualAnalyticalDerivatives) {
  ...
  RunJacobianEval(camera, geometry::camera::ProjectionType::DUAL, &jacobian);
  CheckJacobian(jacobian, size_params);
}
