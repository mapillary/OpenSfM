...
Eigen::Vector4d TriangulateBearingsDLT(
    const Matrix<double, 3, Dynamic> &bs,
    const vector<Matrix<double, 3, 4> > &Rts) {
  int nviews = bs.cols();
...
bp::object TriangulateBearings(const bp::list &Rts_list,
  ...
  libmv::vector<Eigen::Matrix<double, 3, 4> > Rts;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    Eigen::Map<const libmv::Mat> Rt(Rt_array.data(), 4, 3);
    Eigen::Map<const libmv::Mat> b(b_array.data(), 3, 1);
